...
2. REQUEST
   The server receives raw bytes from the browser. We pass the request to request = HTTPRequest(raw_request) in the HTTPRequest class. It splits the headers and the body to extract the method(GET/POST), path(/signup), headers and body(username and password) even without the server running.

3. CONTENT-LENGTH AND CONTENT-TYPE
